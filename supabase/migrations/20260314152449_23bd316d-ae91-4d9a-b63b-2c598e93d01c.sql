...
-- Update fn_finalizar_venda_atomica to compute and store total_cost/total_profit
...
BEGIN
  ...
  -- 1. Create venda (total_cost/total_profit will be set after items)
  ...
  VALUES (_empresa_id, _cliente_id, _vendedor_id, 'finalizada', _subtotal, _desconto_total, _total, _pagamentos, COALESCE(_observacoes, ''), _data_venda, _idempotency_key, 0, 0)
  RETURNING id INTO _venda_id;

  -- 2. Insert sale items and accumulate cost/profit
  ...
  LOOP
    ...
    _item_subtotal := (_item->>'subtotal')::numeric;
    _item_qty := (_item->>'quantidade')::numeric;
    ...
    -- 3. Stock movements (unchanged logic)
    IF _item_qty > 0 THEN
      IF (_item->>'is_kit')::boolean = true AND jsonb_array_length(COALESCE(_item->'kit_itens', '[]'::jsonb)) > 0 THEN
  ...
  -- Update venda with computed profit (bypass protection trigger since we just created it)
  UPDATE public.vendas
  SET total_cost = _computed_total_cost, total_profit = _computed_total_profit
  WHERE id = _venda_id;
