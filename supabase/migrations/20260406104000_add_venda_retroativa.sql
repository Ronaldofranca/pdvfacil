-- =============================================================================
-- Migration: Suporte a Lançamento de Vendas Retroativas
-- =============================================================================

-- 1. Adicionar flag indicando se a venda foi lançada retroativamente
ALTER TABLE public.vendas ADD COLUMN IF NOT EXISTS is_retroativa BOOLEAN DEFAULT false;

-- 2. Atualizar a RPC de finalização de venda para suportar _is_retroativa
-- e propagar a data passada para os movimentos de estoque.
...
BEGIN
  ...
  VALUES (
    ...
    _pagamentos, COALESCE(_observacoes, ''), _data_venda, _idempotency_key,
  ...
  -- Suporta tanto 'crediario' (sem acento) quanto 'crediário' (com acento)
  -- para compatibilidade com versões antigas do frontend
  ...
  THEN
    ...
    IF _valor_restante > 0 THEN
      ...
      LOOP
        ...
        VALUES (
          _empresa_id, _venda_id, _cliente_id,
          i + 1, _valor, 0,
          _venc, 'crediario'
        );
      END LOOP;
