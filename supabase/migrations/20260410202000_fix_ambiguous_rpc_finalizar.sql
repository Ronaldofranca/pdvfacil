-- =============================================================================
-- FIX: RESOLUÇÃO DE AMBIGUIDADE NA FUNÇÃO fn_finalizar_venda_atomica
-- Objetivo: Remover versões obsoletas da função que causam conflito no RPC.
-- NOTA: Este script NÃO altera nenhum dado de venda, estoque ou financeiro.
--       Ele apenas limpa as definições de código duplicadas.
-- =============================================================================

-- 1. Remover versões com assinaturas antigas (assinaturas exatas)
-- Versão com 12 parâmetros (antiga)
DROP FUNCTION IF EXISTS public.fn_finalizar_venda_atomica(text, uuid, uuid, uuid, numeric, numeric, numeric, jsonb, text, timestamptz, jsonb, jsonb);

-- Versão com 13 parâmetros (intermediária)
DROP FUNCTION IF EXISTS public.fn_finalizar_venda_atomica(text, uuid, uuid, uuid, numeric, numeric, numeric, jsonb, text, timestamptz, jsonb, jsonb, boolean);

-- 2. Recriar/Garantir a versão oficial e robusta (15 parâmetros)
