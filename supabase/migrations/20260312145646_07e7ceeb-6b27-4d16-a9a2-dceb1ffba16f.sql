...
-- Fix: Recreate produtos_catalogo view without security_invoker
-- This lets the view run as owner (bypassing anon RLS on base table)
-- while the base table's USING(false) policy still blocks direct anon access to custo
