-- ========================================================
-- 1. CORREÇÃO DA FUNÇÃO ATÔMICA DE FINALIZAÇÃO DE VENDA
-- ========================================================
...
BEGIN
  ...
  -- 0. PRE-COMPUTE total_cost and total_profit (AVOID UPDATE AFTER INSERT)
  ...
  -- 1) Create venda (JÁ COM TUDO CALCULADO)
  ...
  -- 2) Insert sale items
  PERFORM set_config('app.finalizar_venda_atomica', 'on', true); -- Permite bypass se houver trigger de proteção em itens_venda
  FOR _item IN SELECT * FROM jsonb_array_elements(_itens)
  LOOP
    _real_kit_id := NULL;
    ...
    VALUES (
      ...
      _item->>'nome', _item_qty, COALESCE((_item->>'preco_original')::numeric, 0),
      COALESCE((_item->>'preco_vendido')::numeric, 0), COALESCE((_item->>'desconto')::numeric, 0),
      COALESCE((_item->>'bonus')::boolean, false), _item_subtotal, _item_cost
    );
    ...
    IF _item_qty > 0 THEN
      IF COALESCE((_item->>'is_kit')::boolean, false) = true AND jsonb_array_length(COALESCE(_item->'kit_itens', '[]'::jsonb)) > 0 THEN
        ...
        LOOP
          ...
          VALUES (_empresa_id, (_ki->>'produto_id')::uuid, _vendedor_id, 'venda', COALESCE((_ki->>'quantidade')::numeric, 0) * _item_qty,
      ...
      ELSE
        ...
        VALUES (_empresa_id, _produto_id_for_db, _vendedor_id, 'venda', _item_qty, format('Venda #%s', substr(_venda_id::text, 1, 8)));
      END IF;
    END IF;
  END LOOP;
...
-- ========================================================
-- 2. REFINAMENTO DO GATILHO DE PROTEÇÃO (VENDAS)
-- ========================================================
...
BEGIN
  ...
  IF OLD.status = 'finalizada' THEN
    -- 1. Permitir cancelamento
    IF NEW.status = 'cancelada' THEN
      RETURN NEW;
    END IF;

    -- 2. Bloquear alteração de dados base APENAS se houver mudança de fato (IS DISTINCT FROM)
    -- Isso permite que updates acidentais que não mudam valores passem sem erro.
    IF (NEW.total IS DISTINCT FROM OLD.total) OR 
      ...
      RAISE EXCEPTION 'POLITICA DE PROTEÇÃO DE DADOS: O conteúdo base de uma venda finalizada não pode ser alterado. Para correções, cancele esta venda e crie uma nova.';
    END IF;

    -- Permite atualização de campos secundários (pagamentos, obs, custo, lucro, audit logs)
    RETURN NEW;
  END IF;
  
  -- Vendas canceladas continuam estritamente imutáveis
  IF OLD.status = 'cancelada' THEN
    RAISE EXCEPTION 'POLITICA DE PROTEÇÃO DE DADOS: Vendas com status "cancelada" não podem ser modificadas.';
  END IF;
