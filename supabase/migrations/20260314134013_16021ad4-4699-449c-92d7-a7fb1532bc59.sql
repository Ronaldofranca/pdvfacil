...
-- 1. Add idempotency_key column to vendas
ALTER TABLE public.vendas ADD COLUMN IF NOT EXISTS idempotency_key text UNIQUE;

-- 2. Create atomic finalization RPC
CREATE OR REPLACE FUNCTION public.fn_finalizar_venda_atomica(
  ...
  _pagamentos jsonb,
  _observacoes text,
  _data_venda timestamptz,
  _itens jsonb,
  _crediario jsonb DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  _valor numeric;
BEGIN
  -- IDEMPOTENCY CHECK: If this key was already processed, return the existing venda
  ...
  -- 1. Create venda
  INSERT INTO public.vendas (empresa_id, cliente_id, vendedor_id, status, subtotal, desconto_total, total, pagamentos, observacoes, data_venda, idempotency_key)
  VALUES (_empresa_id, _cliente_id, _vendedor_id, 'finalizada', _subtotal, _desconto_total, _total, _pagamentos, COALESCE(_observacoes, ''), _data_venda, _idempotency_key)
  ...
  LOOP
    ...
    VALUES (
      ...
      _item->>'nome',
      (_item->>'quantidade')::numeric,
      (_item->>'preco_original')::numeric,
      (_item->>'preco_vendido')::numeric,
      (_item->>'desconto')::numeric,
      COALESCE((_item->>'bonus')::boolean, false),
      (_item->>'subtotal')::numeric,
      COALESCE((_item->>'custo_unitario')::numeric, 0)
    );

    -- 3. Stock movements
    IF (_item->>'quantidade')::numeric > 0 THEN
      IF (_item->>'is_kit')::boolean = true AND jsonb_array_length(COALESCE(_item->'kit_itens', '[]'::jsonb)) > 0 THEN
        -- Kit: create movement for each component
        ...
        LOOP
          ...
          VALUES (
            ...
            _vendedor_id,
            'venda',
            (_ki->>'quantidade')::numeric * (_item->>'quantidade')::numeric,
...
GRANT EXECUTE ON FUNCTION public.fn_finalizar_venda_atomica TO authenticated;
